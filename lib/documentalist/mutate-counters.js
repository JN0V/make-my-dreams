...
export function mutateCounters(lessons, validatedByLesson, opts = {}) {
  ...
  for (const lesson of lessons) {
    ...
    // Not promotable / nothing new to credit → carry through unchanged. We
    // expose validatedReuseTotal regardless so the CLI can surface it (AC-4).
    if (
    ...
    newlyCreditedRuns[lesson.id] = newRunIds;
