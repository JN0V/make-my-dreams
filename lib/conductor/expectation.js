...
export function writeExpectation(sharedDir, dream, scope, { fs, existsSync, readFileSync: readFileSyncDep, isResume = false } = {}) {
  ...
  // Read the existing oracle's content (to extract its dream-id). When there is no
  // reader seam (old callers) but the file IS there, treat it as an existing oracle
  // of an UNKNOWN dream (a stamp-less placeholder), NOT as "no oracle": the decision
  // then refreshes it on a non-resume launch and preserves it on a resume — the
  // documented backward-compat behavior.
  ...
  if (alreadyThere) {
    if (typeof readFileSyncDep === 'function') {
      try {
        ...
        existingContent = '<unreadable oracle>';
      }
    } else {
      existingContent = '<unknown oracle: no reader seam>';
    }
  }
