// lib/onboarding/setup.js — the transparent first-run setup guard for `mmdream --here`.
//
 ...
 * @param {() => ({ok: boolean, exitCode?: number, message?: string})} [args.preflightFn]
 *        optional gate run AFTER "not ready" but BEFORE writing anything (the
 *        caller checks the working tree is clean, so the post-setup commit can
 *        never sweep the user's pre-existing uncommitted work). Not-ok → abort
 *        with its exitCode, runner never called.
...
export async function runFirstRunSetup({
  ...
  // 3. Not ready — but before we write anything, let the caller veto on a
  // precondition it owns (a dirty working tree): setup materializes files the
  // caller will commit, and committing a tree that was ALREADY dirty would
  // sweep the user's uncommitted work into the setup commit (a §VI honesty
  // miss). Refusing here, before the runner writes, keeps that impossible.
  if (preflightFn) {
    const pf = await preflightFn();
    if (pf && pf.ok === false) {
      if (pf.message) err(pf.message);
      return { ok: false, action: 'blocked', exitCode: pf.exitCode };
    }
  }

  // Explain what's missing, then confirm (TTY) or auto (non-TTY).
