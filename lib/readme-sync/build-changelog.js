...
/**
 * Strip a leading `<tag>` from a subject when the annotation redundantly repeats
 * it (optionally followed by a `-`/`–`/`—` separator), e.g.
 *   ("v0.3.3 — Layer C: …", "v0.3.3") → "Layer C: …"
 * The whole subject being exactly the tag → '' (caller renders (no annotation)).
 *
 * @param {string} subject
 * @param {string} tag
 * @returns {string}
 */
function stripDuplicateTagPrefix(subject, tag) {
  if (!tag) return subject;
  const re = new RegExp(`^${escapeRegExp(tag)}\\s*[-–—]?\\s*`);
...
export async function buildChangelog(opts) {
  ...
  const lines = field.value.split('\n').filter((l) => l.length > 0).map((line) => {
    ...
    let subject = secondTab === -1 ? '' : line.slice(secondTab + 1).trim();
    // Tag annotations conventionally begin with the tag name itself
    // (`v0.3.3 — Layer C: …`). Since we already render `**<tag>** — `, strip a
    // leading duplicate `<tag>` (and its trailing dash/em-dash separator) so the
    // line reads `- **v0.3.3** — Layer C: …`, not `… — v0.3.3 — Layer C: …`.
    subject = stripDuplicateTagPrefix(subject, tag);
