...
// `mmdream --here`. `--monitor` is a boolean (default false) that COMPOSES with
...
// v0.13.a: HANDOFF_FLAGS gate the cooperative auto-handoff at 70% for `mmd` /
// `mmdream --here`. `--auto-handoff` is a boolean (default false) that IMPLIES
// `--monitor` (resolved in bin/mmd.js — the context usage only exists in the
// stream-json spawn). With it, MMD hands the run off to a FRESH successor when
// the orchestrator's context fills (cooperative, never a forced kill), bounded
// by MMD_MAX_HANDOFFS. Absent (the default), the spawn + single-spawn flow are
// byte-for-byte today's. Kept in its own group (not MONITOR_FLAGS) to make clear
// it is a distinct, higher-level opt-in that subsumes the monitor.
