...
//
// HUB SUPPRESSION (precision lever, AC-4): we do NOT traverse transitively
// THROUGH a high-degree "hub" node — a doc like CLAUDE.md / HANDOVER.md that
// references dozens of ADRs would otherwise couple everything to everything (a
// change near it would weak-flag the whole repo). A direct neighbor whose degree
// exceeds HUB_DEGREE is still reported (it is genuinely coupled), but its OWN
// neighbors are not pulled in as weak edges. This keeps the weak layer to tight,
// actionable "these files travel together" couplings rather than hub noise.
//
...
// A direct neighbor with more than this many edges is a "hub" (e.g. a top-level
// doc that references most of the repo). Hubs are reported as direct neighbors
// but are NOT used as transit nodes for weak/transitive edges — see HUB
// SUPPRESSION above. Tuned so genuine clusters (a test importing a few modules,
// an ADR citing a few files) still propagate, while doc hubs do not.
const HUB_DEGREE = 12;
  ...
  try {
    ...
    for (const file of changed) {
      ...
      if (direct) {
        ...
        // Two-hop (transitive) neighbors: a neighbor of a direct neighbor.
        // We do NOT transit THROUGH a hub (high-degree) node — that would couple
        // the whole repo through a top-level doc (precision lever, see header).
        for (const [nb] of direct) {
          if (changedSet.has(nb)) continue;
      ...
      const neighbors = [...best.entries()].map(([to, v]) => ({
