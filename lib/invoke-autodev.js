 ...
 * is set, the greenfield prompt STATES the audience profile, and for the Kid
 * profile it injects the safe-by-default directive (no network / no third
 * parties / offline / age-appropriate / no accounts or UGC). This is what makes
 * the threaded profile meaningful in the actual build rather than a dead env var
 * (observability — L-022). An UNSET MMD_PROFILE leaves the prompt byte-for-byte
 * unchanged (back-compat: every existing greenfield/CI run is unaffected). The
 * full constitution-module binding by profile is a deferred composer slice
 * (SPEC_V03B §4, ADR-023). The custom-prompt short-circuit above means --here
 * is intentionally NOT affected (a dev flow, not an end-user dream).
 */
export function buildPrompt({ dream, slug, demoDir, engine = 'standard', prompt = undefined, env = process.env }) {
  ...
  // v0.3.b AC-5: consume MMD_PROFILE. Gate on PRESENCE first (a non-empty string)
  // so an unset var leaves the prompt unchanged; normalize the present value to a
  // canonical profile so e.g. `kid`/`enfant` still trigger the Kid safety framing.
