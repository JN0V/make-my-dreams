...
export async function invokeAutodev({
  ...
  // lessons file lives at <repoRoot>/docs/lessons-learned.md. By default we
  // resolve it from the launch cwd (the MMD-project root in self-hosting,
  // the user's project root otherwise). The caller can override via
  // `lessonsPath` for tests / brownfield targets.
  //
  // Errors are non-fatal: composer is best-effort observability. ENOENT is
  // already handled inside composeLessons (returns { missing: true }).
  const resolvedLessonsPath =
    lessonsPath || path.join(process.cwd(), 'docs', 'lessons-learned.md');
  let composerResult;
  ...
  // AC-6: persist the composer audit trail (composer.json) next to the run
  // log. Best-effort — a write failure here is non-fatal to the auto-dev run
  // itself (we keep going, just without the sidecar).
  try {
    await mkdir(path.dirname(logPath), { recursive: true });
    await writeComposerAudit(logPath, composerResult);
  } catch {
    // Swallow: composer.json is observability, not load-bearing.
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    // post-hoc grep finds composition activity without parsing JSON.
    try {
