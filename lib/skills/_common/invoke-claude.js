...
export async function invokeClaudeSkill(opts) {
  ...
  // Callers passing raw `args` (legacy path, mostly tests) bypass composition.
  ...
  // AC-6: write composer.json sidecar next to the log path (best-effort —
  // observability data only, never load-bearing for the run itself).
  if (composerResult) {
    try {
      await writeComposerAudit(logPath, composerResult);
    } catch { /* swallow — observability */ }
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    // AC-6: emit composer header as first log line (mirrors invoke-autodev).
    if (composerResult) {
      try {
