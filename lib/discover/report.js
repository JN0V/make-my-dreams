...
function hypothesesAndContradictions(scanData, ingestData, caseLabel) {
  ...
  if (caseLabel === 'brownfield-app') {
    ...
    hypotheses.push(
      ...
      'it. Running `mmd --here` will offer a one-time setup (constitution + auto-dev ' +
...
function suggestedNextStep(caseLabel, ingestData) {
  ...
  if (caseLabel === 'brownfield-app') {
    return 'Review this report, run `mmd discover --approve`, then `mmd --here "<small change>"` — MMD will offer a one-time setup (accept it once) before the first slice.';
  }
