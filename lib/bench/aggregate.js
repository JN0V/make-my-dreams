 ...
 * @param {string} args.mmd_version mmdream version at run time
 * @param {string} args.mmd_git_sha mmdream git short SHA at run time
...
export function buildReportMd(summary) {
  const lines = [];
  lines.push(`# mmdream bench — run ${summary.run_id}`);
  lines.push('');
