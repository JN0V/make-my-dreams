...
//       `mmdream <skill>: '<command>' not found on PATH (override with MMD_<SKILL>_CMD)`.
...
export function assertSkillInstalled({ skillName, skillPath, existsSyncFn = existsSync }) {
  ...
  return {
    ...
    missingPath: abs,
    message:
      `mmdream ${skillName}: gStack '${skillName}' skill not found at ${abs}.\n` +
...
export function maybeWarnConcurrentClaude(opts = {}) {
  ...
  for (const line of lines) {
    ...
    if (Number.isFinite(pid) && pid !== selfPid) {
      write(
        `[mmdream ${skillName}] warning: another 'claude -p' process appears to ` +
...
export async function invokeClaudeSkill(opts) {
  ...
  if (!Array.isArray(opts.args) && typeof opts.prompt === 'string' && opts.prompt.length > 0) {
    ...
    try {
      // SPEC_V02L AC-4: pass the per-skill context so the composer filters by
      // `Applies to`. skillName is the gStack skill (ship/qa/cso/...) → the
      // composer sees `mmdream <skill>` as the subcommand. NOTE: this is the
      // INTERNAL composer-matching key (matched against lessons' `Applies to`),
      // NOT a display string — it stays `mmd` (the project brand) even though the
      // executable is now `mmdream`, so historical lessons keep matching.
      composerResult = await composeLessons(opts.prompt, lessonsPath, {
        context: { subcommand: `mmd ${skillName}` },
      });
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      err.mmdExitCode = 4;
      err.message =
        `mmdream ${skillName}: failed to spawn '${command}': ${err.message}. ` +
        `Install Claude Code or set MMD_${skillUpper}_CMD to a test fixture.`;
      // F28 (Phase-4 re-review): use fail() so the in-flight
    ...
    child.on('error', (err) => {
      if (err && err.code === 'ENOENT') {
        err.mmdExitCode = 4;
        err.message =
          `mmdream ${skillName}: '${command}' not found on PATH ` +
    ...
    if (typeof heartbeatIntervalMs === 'number' && heartbeatIntervalMs > 0) {
      heartbeatTimer = setInterval(() => {
        try {
          ...
          const line = `[mmdream ${skillName}] still running, pid=${child.pid}, ` +
    ...
    if (typeof timeoutMs === 'number' && timeoutMs > 0) {
      killTimer = setTimeout(() => {
        ...
        try { logStream.write(`\n[mmdream ${skillName}] subprocess timed out\n`); }
        catch { /* stream closed */ }
      }, timeoutMs);
    }
