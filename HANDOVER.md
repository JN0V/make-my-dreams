...
- **Tests**: 1998 passing
- **Recent commits**:
  - `dfe79aa fix(dream): raise the dream-length cap 500 → 4000 (500 rejected real first dreams)`
  - `e85c90e docs(/mmdream): note that first run on a fresh repo auto-installs before the slice`
  - `690a00c docs: refresh mechanical blocks (auto-setup for all spawning commands, 1996 tests)`
  - `6ef4e04 fix(onboarding): auto-install on first launch for EVERY command that needs BMAD/gStack`
  - `c9093c5 docs: refresh mechanical blocks (serve/greenfield auto-setup, 1990 tests)`
...
**This session's arc (v0.6.0 → v0.8.0):** third-party readiness → the full Documentalist (`mmdream document-review` detect + drift/conformance + `mmdream document-compact` SPEC archival + `--since` coherence graph + hub-cap) → `/mmdream` operator slash command (live in `.claude/commands/`) → the Test Curator (`mmdream test-health`: corpus health + redundancy + cluster precision) → **the §VIII polyglot correction** (Sébastien caught that the Test Curator was JS-only; fixed via adapters + the new constitution §VIII guardrail; proven on Python, refuses Rust honestly). Each version has a detailed "JUST LANDED" block below and a changelog entry in README.

**§VIII analysis debt — DONE (v0.8.0 Test Curator + v0.8.1 import graph/coherence + v0.8.2 doc→code refs all polyglot/adapter-based, proven on Python, honest on unsupported stacks; guardrail = constitution universal §VIII).** Only **polyglot coverage** remains as a §VIII item, and it is a *new build* (native tool per adapter → lcov), not a JS-only tool to fix.
...
**The Test Curator gained a second face: find tests that likely _overlap_, so the corpus can be pruned.** (ADR-040 deferred "duplicate-test detection"; now built.) Implemented per SPEC_V077.md (frozen), 5 ACs, via this reflexive `mmdream --here` slice. Same contract as the stratification face — **deterministic** (no LLM), **advisory**, **DETECT-BEFORE-CUT** (never deletes a test — a similar-looking test may document distinct intent; the human decides). Method = **clustering by target + structural similarity, NOT coverage** (a coverage mode stays a deferred opt-in — it needs an instrumented run that breaks the pure/read-only contract). What shipped:
...
**The Documentalist now sees COUPLING: change one node, learn which doc/code/ADR neighbors to review — derived from edges that already exist, advisory, never a gate.** Implemented per SPEC_V07D.md (frozen), 5 ACs, via the 32nd reflexive `mmdream --here`. The golden rule: **derive, never maintain** — a hand-kept "these relate" map rots; this graph is composed for free from `computeBlastRadius` (imports), `doc-refs` (doc→code), and the new doc-links (doc↔doc). What shipped:
