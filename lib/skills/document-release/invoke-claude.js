...
export const buildDocumentReleaseArgs = (prompt) => buildSkillArgs(prompt);
...
export function documentReleasePaths(repoRoot, now = () => new Date()) {
  ...
  return {
    outputPath: path.join(dir, `${ts}-${process.pid}.md`),
    logPath: path.join(dir, `${ts}-${process.pid}-driver.log`),
  };
}
