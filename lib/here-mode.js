 ...
 *
 * v0.2a note: this parser is exported and tested but currently has no
 * runtime effect — bin/mmd.js always creates the slice branch regardless
 * of the source branch (the protected-branch case is "always permissive,
 * never blocking" per SPEC_V02A AC-2). The env var is forward-compat
 * documentation for the v0.5 Conductor (MAKE_MY_DREAMS §6.4) which will
 * enforce policy on protected-branch slice creation (e.g. require an
 * explicit confirmation, or restrict who can run `mmd --here` from main).
 * Wiring the parser into the runtime is a separate slice — see
 * v0.2a-deferred-work.md if one was produced.
 *
