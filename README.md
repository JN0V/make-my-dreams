...
- **Reflexive slices (release tags)**: 64
...
The bulleted block above (version, tag, counts) is machine-maintained by `mmdream document`; the prose below is the human-owned story.
...
- [`HANDOVER.md`](./HANDOVER.md) — session handover (state + intent); its State block is auto-refreshed by `mmdream document`
