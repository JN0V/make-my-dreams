...
import { SHARED_MARKER } from './sealed-tests/tester-prompt.js';
 ...
 * explicit confirmation, or restrict who can run `mmdream --here` from main).
 *
 ...
 *
 * v0.4.b — optional `sealedDir`: when set (the `mmdream --here --sealed` flow), a
 * read-only sealed-oracle note is appended, mirroring buildCoderPrompt's block
 * (lib/sealed-tests/tester-prompt.js). It tells the --here coder that the sealed
 * directory holds acceptance tests written by an independent tester BEFORE it
 * ran, and that editing/weakening/deleting any sealed file is a TAMPER that fails
 * the slice (anti-P-04). MMD enforces this by re-hashing regardless of the prompt;
 * the note is the human-readable courtesy half of the contract (universal §VII).
 * Absent (the default --here flow), the prompt is unchanged.
 *
 * @param {{ dream: string, sliceBranch: string, targetDir: string, engine?: 'fast'|'standard', sealedDir?: string }} opts
 * @returns {string}
 */
export function buildHerePrompt({ dream, sliceBranch, targetDir, engine = 'standard', sealedDir }) {
  if (typeof dream !== 'string' || dream.length === 0) {
  ...
  // v0.4.b: sealed-oracle read-only note (mirrors buildCoderPrompt). Only when
  // the --here run is sealed and a sealedDir was passed.
  if (typeof sealedDir === 'string' && sealedDir.trim() !== '') {
    lines.push(
      ...
      '- Make them pass by implementing the CHANGE — write the app/source code, not the tests.',
