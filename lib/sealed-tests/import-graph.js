...
// (AC-1). Block comments first, then line comments to end-of-line. The `//`
// heuristic skips a `//` immediately preceded by `:` so a `http://` / `node://`
// inside a string is not mistaken for a comment — a relative specifier (the
// only kind we resolve) never contains `://`, so this is safe and cheap.
