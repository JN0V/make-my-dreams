...
/**
 * Compare a captured version tuple against the current version tuple.
 * Returns true when `pinned` <= `current` (a forward claim of an already-shipped
 * version is stale). Reuses the same loose 3-part comparison shape as the v0.18
 * version helpers (looseVersion/cmp), here inlined on already-parsed tuples.
 *
...
function versionAtOrBelow(pinned, current) {
  ...
  return true; // equal → at the current version → already shipped/stale.
}
...
function allVersionsAtOrBelow(line, current) {
  ...
  return sawVersion || true; // no version → a pure built-capability claim is removable.
}
