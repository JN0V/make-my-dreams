 ...
 * gives us. PRECONDITION (caller's responsibility): only invoke this on an
 * interactive process with $EDITOR set. MMD's only caller (the CLI driver, reached
 * from bin/mmd.js) is TTY-gated — `--catch` on a non-TTY exits 2 before any
 * dialogue runs — so blocking on stdio:'inherit' is correct there, never a hazard.
 * A non-interactive caller MUST gate on interactivity itself before calling.
 *
