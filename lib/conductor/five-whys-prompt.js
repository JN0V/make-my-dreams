...
// SPEC_V02N AC-4: when this signal is present in the StallContext (routed in by
// bin/conductor/unblock.js via `signals: detection.signals`), the prompt gains
// an additive WIP-salvage hint. It recommends `escalate-to-user` (a human must
// decide what to keep) and names the non-destructive salvage step. It is
// guidance text ONLY — the closed RECOMMENDED_ACTIONS set is unchanged and the
// sacred escalate-to-user fallback (ADR-011, L-016) is reached through the
// existing parser path, never hardcoded here.
const WIP_UNCOMMITTED_SIGNAL = 'wip-uncommitted-since-N-min';

/** Build the additive WIP-salvage hint block (empty unless the signal fired). */
function wipSalvageHint(signals, sliceBranch) {
  if (!Array.isArray(signals) || !signals.includes(WIP_UNCOMMITTED_SIGNAL)) return [];
  return [
    '## WIP-salvage guidance (signal: wip-uncommitted-since-N-min)',
    '',
    'The detector found uncommitted changes in the worktree whose last commit is',
    'older than the WIP threshold — the L-019 WIP-loss-on-kill risk (an auto-dev',
    'run killed mid-flight that left work uncommitted). When this signal is',
    'present, `escalate-to-user` is the appropriate recommended action: a human',
    'must decide what to keep. Before anything else, the non-destructive salvage',
    'step is to stash the uncommitted work so it can never be lost:',
    '',
    '```',
    `git stash push -u -m "wip-salvage ${sliceBranch}"`,
    '```',
    '',
    'This is guidance only — still derive the action from your own 5-why chain and',
    'pick exactly one value from the closed enum. Do NOT invent a new action.',
    '',
  ];
}
...
export function buildFiveWhysPrompt(context) {
  ...
  return [
    ...
    '```',
    '',
    ...wipSalvageHint(signals, sliceBranch),
    '## Your task',
    '',
