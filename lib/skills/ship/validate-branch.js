// lib/ship/validate-branch.js — pre-flight checks for `mmdream ship` (SPEC_V02F AC-3).
//
...
// command string. Branch names provided by the user (via `mmdream ship <branch>`)
...
/**
 * Pure validation of a branch name. Used by both the bare `mmdream ship` path
 * (where the branch comes from `git branch --show-current`) and the explicit
 * `mmdream ship <branch>` path (where the user provides it).
 *
 * Rules:
...
export function validateBranchName(name) {
  if (typeof name !== 'string' || name.length === 0) {
    return {
      ok: false,
      exitCode: 4,
      message: 'mmdream ship: branch name is empty (run from a checked-out branch or pass <branch>).',
    };
  }
  if (/\s/.test(name) || name.startsWith('-')) {
    return {
      ok: false,
      exitCode: 4,
      message: `mmdream ship: refusing branch with suspicious characters: '${name}'`,
    };
  }
  if (isProtectedBranch(name)) {
    return {
      ...
      exitCode: 4,
      message:
        `mmdream ship: '${name}' is a protected branch — refusing to ship FROM it. ` +
  ...
  if (!isAllowedBranchPrefix(name)) {
    return {
      ...
      exitCode: 4,
      message:
        `mmdream ship: branch '${name}' does not match an allowed prefix (` +
...
// runGit lives in lib/skills/_common/git.js (v0.2.g F9 — was duplicated
// across 4 validators).
...
export async function validateShipTarget(cwd, opts = {}) {
  if (typeof cwd !== 'string' || cwd.length === 0) {
    return {
      ok: false,
      exitCode: 3,
      message: 'mmdream ship: internal error — cwd is required',
    };
  }
  ...
  if (!inside.ok) {
    return {
      ...
      exitCode: 3,
      message:
        `mmdream ship requires the current directory to be a git repository, ` +
  ...
  if (!branch) {
    ...
    if (!cur.ok || cur.code !== 0) {
      return {
        ok: false,
        exitCode: 3,
        message: `mmdream ship: could not read current branch: ${
  ...
  if (!rev.ok || rev.code !== 0) {
    return {
      ok: false,
      exitCode: 3,
      message: `mmdream ship: could not read HEAD SHA: ${
