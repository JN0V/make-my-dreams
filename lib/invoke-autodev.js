...
export async function invokeAutodev({
  ...
  // SPEC_V02E AC-4: compose lessons into the prompt BEFORE spawning. The
  // lessons file lives at <repoRoot>/docs/lessons-learned.md.
  //
  // Resolution order (F11 Phase-4 review — use function-argument paths
  // consistently, never process.cwd() which is mutable test state):
  //   1. Explicit `lessonsPath` from the caller — wins.
  //   2. <absDemoDir>/docs/lessons-learned.md — for --here mode this IS the
  //      target repo's lessons; for greenfield this won't exist (demo dir
  //      is brand-new) and we fall through.
  //   3. <parent of absDemoDir>/docs/lessons-learned.md — for greenfield
  //      where absDemoDir = <project-root>/demo/<slug> the parent of demo/
  //      is the MMD-project root carrying docs/lessons-learned.md.
  //   4. composeLessons returns { missing: true } if none of the above
  //      exist — observability path, no error.
  //
  // Composer errors are non-fatal: best-effort observability.
  const candidates = [
    lessonsPath,
    path.join(absDemoDir, 'docs', 'lessons-learned.md'),
    path.join(path.dirname(absDemoDir), 'docs', 'lessons-learned.md'),
  ].filter(Boolean);
  const resolvedLessonsPath =
