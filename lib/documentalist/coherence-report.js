...
function renderDrift(drift) {
  ...
  const scannedUx = typeof drift?.scannedUx === 'number' ? drift.scannedUx : 0;
  ...
  lines.push(
    ...
    `(detect-before-correct). Scanned ${scanned} truth doc${scanned === 1 ? '' : 's'}` +
    `${scannedUx > 0 ? ` + ${scannedUx} UX-text surface${scannedUx === 1 ? '' : 's'} (scripts + --help)` : ''}._`,
  );
  lines.push('');
  ...
  // Deprecated surface (SPEC_V018A AC-3): a known-stale token recommended as the
  // current/primary surface (scanned across markdown + scripts + --help, AC-2).
  const deprecated = Array.isArray(drift?.deprecated) ? drift.deprecated : [];
  lines.push('### Deprecated surface (a stale token recommended as the current entry)');
  lines.push('');
  if (deprecated.length === 0) {
    lines.push('- ✅ No deprecated-surface recommendations — no known-stale token is presented as primary.');
  } else {
    for (const d of deprecated) {
      lines.push(`- ⚠️ ${cell(d.doc)}:${d.line} recommends \`${cell(d.token)}\` — ${cell(d.reason)} (use ${cell(d.replacement)})`);
    }
  }
  lines.push('');

  // Stale promises (SPEC_V018A AC-4): a version-pinned future promise that came due.
  const stalePromises = Array.isArray(drift?.stalePromises) ? drift.stalePromises : [];
  lines.push('### Stale promises (a version-pinned promise that came due)');
  lines.push('');
  if (stalePromises.length === 0) {
    lines.push('- ✅ No stale promises — no "to be added in vX" / "coming in vX" is past due.');
  } else {
    for (const p of stalePromises) {
      lines.push(
        `- ⚠️ ${cell(p.doc)}:${p.line} promises "${cell(p.promise)}" but the current version is ` +
        `${cell(p.currentVersion)} (>= ${cell(p.pinnedVersion)}) — the promise came due.`,
      );
    }
  }
  lines.push('');
