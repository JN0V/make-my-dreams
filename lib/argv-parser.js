...
// short human-readable branch name for `mmd --here` instead of letting the slug
...
// `mmd "<dream>"`. `--catch` forces the dialogue (errors on a non-TTY), `--no-catch`
...
// `mmd "<dream>"`. `--sealed` is a boolean (default false) that COMPOSES with
...
// v0.5.b: MONITOR_FLAGS gate the opt-in live context monitor for `mmd` /
// `mmd --here`. `--monitor` is a boolean (default false) that COMPOSES with
// every other flag — there is no mutex. It switches auto-dev to a stream-json
// spawn so MMD can parse `usage` live and report the orchestrator's context %.
// Absent (the default), the spawn is byte-for-byte the current text path — the
// one that builds MMD itself, so it must stay untouched (SPEC_V05B §1,
// bootstrap safety). Kept in its own group (not MODE_FLAGS) to make clear it
// does not mutex against --here or the engine flags.
const MONITOR_FLAGS = Object.freeze(['monitor']);
const KNOWN_FLAGS = Object.freeze([
  ...ENGINE_FLAGS, ...SESSION_FLAGS, ...MODE_FLAGS, ...VALUE_FLAGS, ...CATCH_FLAGS,
  ...SEALED_FLAGS, ...MONITOR_FLAGS,
]);
...
// helper is parseDocumentReadmeArgs below. It applies the `mmd handover` pattern
...
const SUBCOMMANDS = Object.freeze([
  ...
  'document-lessons', 'handover', 'document-readme', 'document-review',
  'document-compact', 'test-health', 'secret-scan',
]);

export { ENGINE_FLAGS, SESSION_FLAGS, MODE_FLAGS, VALUE_FLAGS, CATCH_FLAGS, SEALED_FLAGS, MONITOR_FLAGS, KNOWN_FLAGS, SUBCOMMANDS };
...
/**
 * v0.3.b AC-3 — resolve whether the greenfield `mmd "<dream>"` path should run
...
/**
 * Parse `mmd unblock` subcommand args (SPEC_V02J AC-3).
 *
 * Recognized tokens:
...
export function parseUnblockArgs(rawArgs) {
  ...
  for (let i = 0; i < rawArgs.length; i += 1) {
    ...
    if (typeof tok === 'string' && tok.startsWith('--')) {
      out.error = {
        message: `unknown unblock arg: '${tok}'. Run 'mmd unblock --help' to see supported flags.`,
        exitCode: 2,
      };
      return out;
    }
...
/**
 * Parse `mmd handover` subcommand args (SPEC_V02P AC-1).
 *
 * Recognized tokens:
...
export function parseHandoverArgs(rawArgs) {
  ...
  for (let i = 0; i < rawArgs.length; i += 1) {
    ...
    out.error = {
      message:
        `unknown handover arg: '${tok}'. Run 'mmd handover --help' to see supported flags.`,
      exitCode: 2,
    };
    return out;
  }
...
/**
 * Parse `mmd document-readme` subcommand args (SPEC_V03D AC-1).
 *
 * `mmd document-readme` is `mmd handover` applied to README.md, so its argv is
...
export function parseDocumentReadmeArgs(rawArgs) {
  ...
  for (let i = 0; i < rawArgs.length; i += 1) {
    ...
    out.error = {
      message:
        `unknown document-readme arg: '${tok}'. Run 'mmd document-readme --help' to see supported flags.`,
      exitCode: 2,
    };
    return out;
  }
