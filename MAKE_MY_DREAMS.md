...
**v0.2.g (2026-05-18) — Medium gStack walking skeleton (per L-012 taxonomy).** v0.2.f closed L-012 with ONE skill (`/ship`). v0.2.g extends to FOUR skills and extracts the reusable shape: three new wrappers (`mmd qa`, `mmd cso`, `mmd document-release`) modelled on `mmd ship`, all sharing the new `lib/skills/_common/invoke-claude.js` (PATH-forcing + race-safe log-stream finish + heartbeat + ENOENT mapping). The architecture is `lib/skills/<name>/{build-prompt,validate-input,summary,invoke-claude}.js` + `bin/skills/<name>.js` — every wrapper costs ~200 lines split across 5 small SRP-clean modules. After v0.2.g, the 5th gStack skill is a 1-hour exercise rather than a 1-week design problem. `audit-pillars.sh` now reports gStack invocations across 4 distinct skill names, taking the L-012 gap from "1 of 41" to "4 of 41" (10%). The "Heavy" option (folding skills inside `auto-dev`'s pipeline, orchestrated by a Conductor) is still deferred to v0.5+. The three new subcommands are read-only / advisory and structurally bypass the v0.2c Project Onboarder validation gate so a fresh brownfield can run `mmd cso` to learn what's there. See [ADR-009](./docs/adr/009-medium-gstack-integration-pattern.md) for the design rationale and [docs/specs/SPEC_V02G.md](./SPEC_V02G.md) for the 7 ACs.
...
**v0.2.j (2026-05-18) — the stuck-recovery primitive.** The Conductor's hardest job is recognizing when a Worker is stuck rather than merely slow, and doing something smarter than "retry" or "kill" (every one of L-004, L-006, L-016 was a silent stuck-and-burn). v0.2.j delivers the walking skeleton of that capability ahead of the full Conductor: a deterministic, sub-100ms **stall detector** (`lib/conductor/stall-detector.js`) that reads `status.json` + last-commit age + run-log error patterns and emits signals from a closed enum, and a **5-Whys session runner** (`lib/conductor/five-whys.js`) that spawns a BMAD Party Mode session (Mary leads the why-chain; Winston/Quinn/Amelia/Christie augment) to diagnose the root cause and recommend one of five closed actions. The composer (v0.2e) auto-injects relevant past lessons into the session prompt, so the system learns from its own stalls. Today this is invoked manually via `mmd unblock <slice>`; it does NOT auto-trigger from a running Worker nor auto-execute the recommended action — those are v0.5+ Conductor concerns. But the exit-code contract (6 escalate / 7 abandon / 8 continue|complete|false-positive) is forward-compatible, so when the Conductor becomes a first-class loop it can detect a stall, call the same detector, run the same session, and branch on the recommendation without re-parsing. See [ADR-011](./docs/adr/011-five-whys-escalation.md) and [docs/specs/SPEC_V02J.md](./SPEC_V02J.md).
...
**v0.2.l added scale-resilience** to conditional injection — the missing half of "not all lessons are injected into all prompts". Where v0.2e selected lessons by keyword overlap alone (global over the whole file, capped at top-5), v0.2.l adds a **context-aware filter that runs BEFORE keyword matching**, mirroring the constitution's per-context `constitution-bindings.yaml` model. Each lesson gains a `**Category**:` folksonomy and an `**Applies to**:` subcommand list; `composeLessons(prompt, lessonsPath, { context })` keeps only the lessons whose `Applies to` includes the current `subcommand` (or `*`), so a `mmd qa` prompt is never polluted by brownfield-only lessons, and a `mmd --here` prompt is never spammed with ship-only ones. This is a predictive capture (L-018, the 5th L-009-pattern echo): closed before the lessons count crosses the threshold where keyword-coincidence collisions start crowding genuinely-relevant lessons out of the top-5 cap. Backward-compatible by default (no context → v0.2e behavior). The `composer.json` audit gains `context`, `filtered_out_by_context`, `matched_by_keyword`, and `injected` metrics. See [ADR-012](docs/adr/012-composer-categorization.md) and SPEC_V02L.
...
**Date**: 2026-05-17. **Slice**: `slice/v0.2a-here-mode`. **Spec**: [docs/specs/SPEC_V02A.md](./SPEC_V02A.md). **ADR**: [005](./docs/adr/005-here-mode-explicit-flag-not-auto-detect.md).
...
|---|---|---|---|
| **A** | **Alignment gate on the normal path** — run the existing v0.4.d judge on `mmdream --here` + greenfield (default-on), grade the impl against the ask, **iterate** (bounded) on a gap; honest exit 7 if the gap survives, never a fabricated pass | highest value/cost ratio; the brick exists; independent of the monolith | ✅ **DONE — v0.11.0** (ADR-049; default-on, `MMD_SKIP_ALIGN=1` opt-out; dogfoods every `--here` run) |
| **B** | **Externalized checkpoint/handoff state + a resumable orchestrator** — auto-dev checkpoints each phase (`checkpoint.json` + `handoff/N.md`); a REAL `--resume` continues from it | the pivot: makes A persistent AND unlocks C + D | ✅ **DONE — v0.12.0** (B.1, ADR-050; auto-dev is now stateless/resumable; `--resume` continues a run) |
| **C** | **Auto-handoff@70%** — the Conductor *acts* on the `READY_FOR_HANDOFF` signal the v0.5.b monitor writes (terminate at a checkpoint + relaunch a resuming successor) | the monitor already sees 70%; only the action was missing | ✅ **DONE — v0.14.0** (HYBRID incite+enforce, ADR-053; PROVEN LIVE both paths). NB: v0.13.0's cooperative-only stop did NOT fire live (LLM ignored it — caught by live verification); v0.14.0 added the deterministic enforce backstop. **Immediate next: the *transparent flip*** — make it default-on (`MMD_NO_AUTO_HANDOFF=1` opt-out) so the user never passes a flag (SPEC_V014A, deferred until now-proven C). |
| **D** | **Parallel Conductor + worktrees + safety hooks** (the old v0.9) — multiple Orchestrators on independent slices, independence pre-flight, semantic-conflict merger | the scalability payoff | planned (needs worktree-safe `unblock` tests — [[worktree-tests-gitignored-fixtures]]) |
...
> **Status (2026-06-04, end of the long Conductor session):** A→B→C are **built + proven live**. The Conductor now (1) verifies the implementation against the ask + iterates (A), (2) checkpoints + resumes a killed orchestrator (B), and (3) auto-hands-off at the threshold — gently when the agent cooperates, by a deterministic SIGTERM-at-checkpoint when it doesn't (C). The whole arc was driven by the **"always verify / untested = broken"** discipline, which caught a false-handoff bug (v0.13.1), a version-pin regression (v0.13.2), and — critically — that the **cooperative-only handoff did not work live** (the green test suite was lying via fakes), forcing the hybrid redesign proven in v0.14.0. **Next, in order:** the transparent flip (default-on auto-handoff) · a small `contextWindowFor('sonnet')` accuracy fix (the monitor mis-estimates Sonnet's window — % wrong, runs still complete) · then D (parallel) and E (Bundle C). Each new capability ships only after a live proof, not a green-by-fakes pass.
...
4. **Execute v0.1** (BOOTSTRAP.md Steps 7–9, 1–2 days): launch `auto-dev` with [`docs/specs/SPEC_V01.md`](./SPEC_V01.md) as input, validate the drawing-app-camera-overlay demo, commit and tag `v0.1.0`.
