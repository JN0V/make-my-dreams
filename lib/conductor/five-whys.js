...
import { modelForRole } from './model-policy.js';
import {
  buildSkillEnv,
  invokeClaudeSkill,
  skillLogPath,
...
export async function runFiveWhys(opts) {
  ...
  try {
    // SPEC_V02L AC-4: the 5-Whys session is the `mmdream unblock` subcommand,
  ...
  // v0.16.a (AC-2): model-per-role — pass the unblock session's policy model to
  // this OWN claude -p call (MMD owns the spawn → --model is honored). Appended
  // AFTER the prompt so the strict positional contract (-p / --output-format /
  // text / prompt) the fake asserts is preserved; omitted when the policy → null.
  const args = ['-p', '--output-format', 'text', prompt];
  const unblockModel = modelForRole('unblock', env);
  if (unblockModel) args.push('--model', unblockModel);
