// lib/skills/ship/build-prompt.js — pure function that constructs the gStack-
// ship invocation prompt sent to `claude -p` (SPEC_V02F AC-4 + AC-5, refactored
// per SPEC_V02G §5).
//
...
// element to spawn() with shell=false (see lib/skills/_common/invoke-claude.js).

import { resolveSkillPath } from '../_common/skill-path.js';

// v0.2.g G2 — module-load-time read of MMD_GSTACK_SKILLS_DIR via the shared
// resolver. When the env var is unset, returns the literal `~/.claude/skills/
// gstack/ship/SKILL.md` byte-identical to the v0.2.f constant. This preserves
// the F4 Option B snapshot stability across the refactor.
const SHIP_SKILL_PATH = resolveSkillPath('ship');
