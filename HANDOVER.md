...
**IMMEDIATE NEXT — THE CORE (the remaining MMD-completeness features). Start with AUTOLEARNING (differentiator #2, §6.5):** close the loop from "lesson injected" → "lesson VALIDATED as having helped" → promote-after-N-validated-reuses + archive-after-M-months. Today the composer injects lessons by keyword (v0.2.e) and `mmd document-lessons` can promote (it auto-promoted L-002/L-016), but the **counter tracks INJECTIONS, not validated reuses** — ADR-010 flagged this and named the Documentalist (now BUILT) as the piece that closes it. The crux to decide first: **how to define a "validated reuse"** (LLM/Documentalist judgment that the rule was followed + helped, vs a deterministic proxy like "a slice that had lesson L injected shipped green in L's category"). Then: Bundle A security, polyglot Reality Check (+ un-skip it in `--here`), auto-handoff@70% (the big one — auto-dev is monolithic), Bundle C/HITL. See NEXT PRIORITY for the full core list.
