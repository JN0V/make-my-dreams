...
// v0.13.a — the cooperative auto-handoff REQUEST marker (SPEC_V013A AC-2). A
// small clearable file the MONITOR writes at the 70% threshold and the auto-dev
// orchestrator cheaply checks at each phase boundary; MMD clears it before each
// successor relaunch. Lives beside the checkpoint in the same run-local area.
const HANDOFF_REQUEST_FILE = 'handoff-request';

/**
 * Coerce to a finite number, returning NaN for anything that cannot convert —
 * crucially WITHOUT throwing (raw `Number(aSymbol)` throws a TypeError). Keeps
 * the never-throws contract honest even for hostile inputs (a Symbol field).
 */
...
/** Path of the cooperative handoff-request marker (v0.13.a). Pure. */
export function handoffRequestPath(dir) {
  return path.join(String(dir ?? '.'), LOCAL_DIR, HANDOFF_REQUEST_FILE);
}
...
export function isResumable(checkpoint, { totalPhases } = {}) {
  ...
  const n = safeNumber(checkpoint.lastCompletedPhase);
...
export function decideResume(args) {
  // Tolerate a null/undefined arg without throwing (the `= {}` default only
  // catches undefined). Keeps the never-throws contract honest.
  const { checkpoint, totalPhases, processAlive = false, statusState } = args || {};
  if (!checkpoint) {
    return { action: 'none', reason: 'no resumable run found (no checkpoint)' };
  }
  if (!isResumable(checkpoint, { totalPhases })) {
    // Distinguish "nothing started yet" (phase ≤ 0) from "all phases done"
    // (phase ≥ totalPhases) so the human reader gets accurate wording (§VII).
    const n = safeNumber(checkpoint.lastCompletedPhase);
    const reason = Number.isFinite(n) && n >= 1
      ? 'nothing to resume — last run completed'
      : 'nothing to resume — no phase completed yet';
...
// ── v0.13.a — cooperative handoff-request marker (SPEC_V013A AC-2) ───────────
// A tiny file the MONITOR writes when the orchestrator's context first crosses
// the handoff threshold, and the auto-dev orchestrator cheaply checks at each
// phase boundary (B.1) to decide whether to stop cleanly for a fresh successor.
// MMD clears it before each relaunch so the successor doesn't immediately
// re-stop. All injected-fs, NEVER throw — best-effort run state, exactly like
// writeCheckpoint (§VI: a failed write is reported via the boolean, never faked).

/**
 * Write (or idempotently overwrite) the handoff-request marker. The payload is
 * small JSON metadata about WHY the handoff was requested (the crossing
 * context %, threshold, model) for forensic value; only its PRESENCE matters to
 * the orchestrator. Re-crossing simply overwrites — it never duplicates.
 *
 * @param {string} dir  run root (absTargetDir for --here, demoDir for greenfield)
 * @param {object} [payload]
 * @param {{fs?:object}} [deps]
 * @returns {boolean} true iff written
 */
export function writeHandoffRequest(dir, payload = {}, { fs = nodeFs } = {}) {
  try {
    const target = handoffRequestPath(dir);
    fs.mkdirSync(path.dirname(target), { recursive: true });
    let body;
    try {
      body = JSON.stringify({ ...payload, requested_at: new Date().toISOString() }, null, 2) + '\n';
    } catch {
      // A non-serializable payload (e.g. a circular object) must not defeat the
      // marker — its PRESENCE is what matters. Fall back to a minimal body.
      body = JSON.stringify({ requested_at: new Date().toISOString() }, null, 2) + '\n';
    }
...
/**
 * Read the handoff-request marker. Returns the parsed payload object when the
 * marker exists (even if its body is unparseable → `{}`, since PRESENCE is the
 * signal), or `null` when there is no marker. NEVER throws.
 *
 * @param {string} dir
 * @param {{fs?:object}} [deps]
 * @returns {object|null}
 */
export function readHandoffRequest(dir, { fs = nodeFs } = {}) {
  try {
    const raw = fs.readFileSync(handoffRequestPath(dir), 'utf8');
    try {
      const obj = JSON.parse(raw);
      return obj && typeof obj === 'object' && !Array.isArray(obj) ? obj : {};
    } catch {
      return {}; // present but malformed — presence is the signal
    }
  } catch {
    return null; // absent (ENOENT) or unreadable → no request
  }
}

/**
 * Has a handoff been requested? Convenience boolean over readHandoffRequest.
 * NEVER throws.
 *
 * @param {string} dir
 * @param {{fs?:object}} [deps]
 * @returns {boolean}
 */
export function handoffRequested(dir, deps) {
  return readHandoffRequest(dir, deps) != null;
}

/**
 * Clear the handoff-request marker. Idempotent: clearing a missing marker is a
 * no-op success. NEVER throws. Returns true iff the marker is absent afterward
 * (either removed or never there).
 *
 * @param {string} dir
 * @param {{fs?:object}} [deps]
 * @returns {boolean}
 */
export function clearHandoffRequest(dir, { fs = nodeFs } = {}) {
  try {
    fs.rmSync(handoffRequestPath(dir), { force: true });
    return true;
  } catch {
    return false;
  }
