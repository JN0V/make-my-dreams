// lib/skills/cso/validate-input.js — pre-flight checks for `mmdream cso`
...
export async function validateCsoTarget(cwd, opts = {}) {
  if (typeof cwd !== 'string' || cwd.length === 0) {
    return { ok: false, exitCode: 3, message: 'mmdream cso: internal error — cwd is required' };
  }
  ...
  if (!inside.ok) {
    return {
      ...
      exitCode: 3,
      message:
        `mmdream cso requires the current directory to be a git repository, ` +
  ...
  if (!branch) {
    ...
    if (!cur.ok || cur.code !== 0) {
      return {
        ok: false,
        exitCode: 3,
        message: `mmdream cso: could not read current branch: ${
  ...
  if (!branch || branch.length === 0) {
    return {
      ...
      exitCode: 3,
      message:
        'mmdream cso: current branch is empty — detached HEAD? ' +
  ...
  if (/\s/.test(branch) || branch.startsWith('-')) {
    return {
      ok: false,
      exitCode: 3,
      message: `mmdream cso: refusing branch with suspicious characters: '${branch}'`,
    };
  }
  ...
  if (!rev.ok || rev.code !== 0) {
    return {
      ok: false,
      exitCode: 3,
      message: `mmdream cso: could not read HEAD SHA: ${
