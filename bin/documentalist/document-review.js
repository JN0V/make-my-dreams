...
Behavior:
  ...
  heuristic: built / partial / unbuilt), and writes a regenerable coherence
  report to ${REPORT_REL_PATH}. Prints a short summary.
...
function gatherRealInventory(root) {
  ...
  return gatherInventory({
    readFile, readDir, listTags, parseLessons, subcommands: [...SUBCOMMANDS],
  });
}
...
export async function runDocumentReview(rawArgs, injected = {}) {
  ...
  const report = renderCoherenceReport({ inventory, reconciliation, llm, version: VERSION });
