#!/usr/bin/env node
// bin/test-curator/test-health.js — `mmdream test-health` entry point (SPEC_V076
...
const USAGE = `mmdream test-health — the Test Curator's POLYGLOT test-corpus health review (SPEC_V080)

Usage:
  mmdream test-health [--dry-run]
...
export function parseTestHealthArgs(rawArgs) {
  ...
  for (const tok of rawArgs) {
    if (tok === '--dry-run') out.dryRun = true;
      ...
      out.error = {
        message: `unknown test-health arg: '${tok}'. Run 'mmdream test-health --help' to see supported flags.`,
        exitCode: 2,
      };
      return out;
    }
...
function wrapReport(body) {
  return [
    '<!-- GENERATED by `mmdream test-health` — regenerate after material test changes; do NOT hand-edit. -->',
    '# MMD Test Health',
    '',
    `_MMD ${VERSION} · generated by \`mmdream test-health\` — the Test Curator (polyglot, detect-and-report only)._`,
    '',
    body,
  ].join('\n');
}
...
export async function runTestHealth(rawArgs) {
  ...
  if (trackedFiles === null) {
    stderr.write(
      ...
      '  mmdream test-health scans the corpus via `git ls-files`; it needs a git repo.\n',
    );
    return 5;
  }
