 ...
 * @param {{ ts?: string, runId?: string }} [opts]
 * @returns {object}
 */
export function buildComposerAudit(composerResult, opts = {}) {
  ...
  const audit = {
    ...
    // SPEC_V02L AC-7: context-aware filtering metrics (additive). `context`
    // is the arg the caller passed (null for legacy/unfiltered calls). The
    // three counts satisfy injected ≤ matched_by_keyword ≤ active − filtered.
    context: composerResult?.context ?? null,
    filtered_out_by_context: composerResult?.filteredOutByContext ?? 0,
    matched_by_keyword: composerResult?.matchedByKeyword ?? matched.length,
    injected: composerResult?.injected ?? matched.length,
    ts: opts.ts || new Date().toISOString(),
  };
  // SPEC_V090 AC-2: stamp a stable per-run id so an audit can be joined to its
  // run outcome (lib/autolearn/run-outcome.js) for the validated-reuse counter.
  // Additive — absent when no runId is supplied (back-compat with v0.2e audits;
  // aggregate-injections + buildRunRecords fall back to the file basename).
  if (typeof opts.runId === 'string' && opts.runId.length > 0) audit.run_id = opts.runId;
...
/**
 * Derive the per-run id from a run-log path: the basename without `.log`. The
 * composer.json and the outcome.json share this key with the log. Pure.
 * @param {string} logPath
 * @returns {string}
 */
function runIdFromLogPath(logPath) {
...
export async function writeComposerAudit(logPath, composerResult) {
  ...
  const audit = buildComposerAudit(composerResult, { runId: runIdFromLogPath(logPath) });
