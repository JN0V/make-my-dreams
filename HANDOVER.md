...
- **IMMEDIATE NEXT → Bundle A Security** (`§6.6`): never built. Worker-sandbox egress allowlist, a `deps-gate` worker (verify a dependency exists + age + downloads before install), pre-commit secret-scanning hooks (trufflehog/git-secrets). Smaller + concrete; real risk for autonomous dev. Then: **polyglot Reality Check** (orchestrate /qa /cso /design-review /canary by deliverable type + un-skip it in `--here`), **auto-handoff@70%** (the big design — auto-dev is monolithic today), **Bundle C/HITL** (per-action risk-scoring, OTel). See NEXT PRIORITY for the full list.
