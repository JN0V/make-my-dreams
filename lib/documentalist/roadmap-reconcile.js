...
//   • a compound capability with some concepts matched, some not → partial
//     (the honest middle: "part of this shipped, part didn't")
//   • only a weak/related token signal                          → partial
...
export function reconcileRoadmap(args) {
  ...
  const classified = entries.map(({ version, capability }) => {
    ...
    if (strong.length === verdicts.length && strong.length > 0) {
      ...
      status = 'partial'; // mixed compound: some concepts shipped, some not
    } else if (weak.length > 0) {
      status = 'partial';
    } else if (versionHasTag(version, signals.tags)) {
      status = 'partial'; // version shipped but no capability-name signal
      labels.push(`tag ${version}.x shipped`);
    } else {
      status = 'unbuilt';
    }
