...
// makes `mmd test-health` produce a genuine, honest report on a Python repo —
...
// A test declaration: a `def test_*(` at any indentation (module-level pytest
// function OR an indented unittest `class Test*` method — both are `def test_*`).
// We capture the function name as the human title. `async def` is allowed.
...
function scanOne(relPath, content) {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (dm) {
      ...
      pendingStratum = null; // consumed by this def
      continue;
    }
