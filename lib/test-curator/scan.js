...
// backtick). `\b` avoids matching `awaitit(`/`subtest(`; we capture the quote so
// we can read the title that follows. Global so multiple calls on one line count.
const TEST_CALL_RE = /\b(?:test|it)\s*\(\s*(['"`])/g;
