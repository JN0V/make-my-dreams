 ...
 * `redacted` are the canonical "fill me in" stand-ins. */
...
// auth/key) assigned a quoted value. The value is captured (group 3) for the
// entropy + length + placeholder checks. `key` is intentionally last/loosest —
// the entropy gate is what makes it precise.
const GENERIC_ASSIGN_RE =
  /\b([A-Za-z0-9_]*(?:secret|password|passwd|pwd|api[_-]?key|access[_-]?key|client[_-]?secret|auth[_-]?token|token|apikey|authkey|key)[A-Za-z0-9_]*)\s*[:=]\s*(['"])([^'"\n]+)\2/gi;
...
export function scanText(text, opts = {}) {
  ...
  const push = (rule, confidence, index, secret) => {
    ...
    if (isPlaceholder(secret, placeholders)) return; // precision: example/placeholder
  ...
  while ((g = GENERIC_ASSIGN_RE.exec(text)) !== null) {
    const value = g[3];
    // The match index points at the identifier; the secret is the quoted value.
    // Report the value's column (where the quoted secret begins) for usefulness.
