...
export function parseReply(reply) {
  ...
  // Tagged SCOPE → strip the marker from the MATCHED head line specifically
  // (F2), then keep every following line so a multi-line scope survives. We
  // operate on the first line we matched on — not a whole-text indexOf, which
  // could (in theory) latch onto a later "SCOPE:" occurrence — so the stripping
  // is self-evidently the same line the `head.startsWith` branch fired on.
  if (head.startsWith(SCOPE_MARKER)) {
    const lines = text.split('\n');
    // `head` is the first NON-EMPTY line; find its index to splice in place.
    const headIdx = lines.findIndex((line) => line.trim().length > 0);
    lines[headIdx] = lines[headIdx].replace(SCOPE_MARKER, '');
