...
The matcher is a case-insensitive word-boundary regex over the prompt text, scored by the count of distinct lesson keywords present. Pure function, no LLM call, no embedding model, no network. Sub-100ms on the live 13-lesson file (measured at ~15ms during v0.2e dev).
...
v0.2e is the **fourth use of `mmd --here`** to develop MMD (after L-010 trivial, L-011 dream-bench feature, L-013 wrapper-modifying). If v0.2e lands clean, the pattern is solid enough to elevate "`mmd --here` is the supported workflow" from rule-in-lessons (L-011) to explicit statement in `commit-git.md` — see SPEC_V02E §6 DoD #12.
