...
import { createSession } from './dream-catcher/session.js';
...
export async function createServer({ port, explicitPort, cwd }) {
  ...
  async function readCatchBody(req, res) {
    const declaredLen = Number(req.headers['content-length']);
    if (Number.isFinite(declaredLen) && declaredLen > MAX_BODY_BYTES) {
      jsonResponse(res, 413, { error: 'body_too_large', max_bytes: MAX_BODY_BYTES });
      return null;
    }
    let body;
    try {
  ...
  async function handleCatchEdit(req, res, requestId) {
    ...
    if (scope.length > MAX_DREAM_LEN) {
      return jsonResponse(res, 400, { error: 'scope_too_long', max_chars: MAX_DREAM_LEN });
    }
    let r;
    try {
