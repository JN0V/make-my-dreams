...
import { suggestConstitutionImprovements } from './constitution-suggest.js';
...
const CASE_LABELS = Object.freeze({
  ...
  'brownfield-app': 'Brownfield app (recognized stack, no SDD methodology)',
...
function scannedSection(scanData) {
  ...
  return [
    ...
    `- Frameworks: ${fr}`,
    `- Test runner: ${runner}`,
    `- Tests: ${testCount} file(s)${testDirs}`,
    `- Git: ${gitLine}`,
  ];
}
...
function hypothesesAndContradictions(scanData, ingestData, caseLabel) {
  ...
  if (caseLabel === 'brownfield-app') {
    const f = scanData.frameworks || {};
    const langs = scanData.languages || { top5: [] };
    const stack = f.language || (Array.isArray(langs.top5) ? langs.top5[0] : null) || 'an unrecognized-but-present stack';
    hypotheses.push(
      `A real app was detected (${stack}) but no SDD methodology — MMD can onboard ` +
      'it. Running `mmdream --here` will offer a one-time setup (constitution + auto-dev ' +
      'workflow) before the first slice. An existing constitution is never overwritten.',
    );
    hypotheses.push(
      'Validate that the inferred test runner + lint setup matches reality before ' +
      'approving — auto-dev relies on the inferred stack section.',
    );
  }
...
function suggestedNextStep(caseLabel, ingestData) {
  if (caseLabel === 'already-onboarded') {
    return 'Run `mmdream discover --refresh` if anything material changed, or proceed with `mmdream --here "<change>"`.';
  }
  if (caseLabel === 'rich' && ingestData && ingestData.spec_kit_constitution) {
    return 'Validate the imported Spec Kit constitution at `.mmd/shared/constitution/imported.md`, then run `mmdream discover --approve`.';
  }
  if (caseLabel === 'bmad-alone') {
    return 'Triage the stories at `.mmd/shared/status.json` (archive stale ones), then run `mmdream discover --approve`.';
  }
  if (caseLabel === 'brownfield-app') {
    return 'Review this report, run `mmdream discover --approve`, then `mmdream --here "<small change>"` — MMD will offer a one-time setup (accept it once) before the first slice.';
  }
  // blank or rich-without-constitution.
  return 'Review this report, then run `mmdream discover --approve` followed by `mmdream --here "<small change>"` to test.';
}

/**
 * Build the "Constitution suggestions" section lines (SPEC_V06B AC-2).
 *
 * Advisory + non-destructive ("elle reste"): present ONLY when the orchestrator
 * read a constitution (constitutionText is a non-empty string). When no
 * constitution exists the caller passes null/undefined and this returns [] so
 * the section is omitted entirely — no noise; the first-run setup materializes a
 * default in that case. The note explicitly states (a) the file is never
 * modified and (b) the list is a HEURISTIC, never an authoritative audit
 * (universal.md §VI honesty).
 *
 * @param {string|null|undefined} constitutionText
 * @returns {string[]}  section lines, or [] to omit the section
 */
function constitutionSuggestionsSection(constitutionText) {
  if (typeof constitutionText !== 'string' || constitutionText.trim().length === 0) {
    return [];
  }
  const { present, missing } = suggestConstitutionImprovements(constitutionText);
  const lines = [];
  lines.push('## Constitution suggestions (advisory — your constitution is never modified)');
  lines.push(
    'Heuristic only — MMD looked for common governance themes by keyword and ' +
    'reports what it did and did not find. This is NOT an authoritative audit, ' +
    'and your `.specify/memory/constitution.md` is left untouched ("elle reste").',
  );
  if (present.length > 0) {
    lines.push(`Your constitution looks solid on: ${present.join(', ')}.`);
  } else {
    lines.push('None of the common governance themes MMD checks were detected by keyword.');
  }
  if (missing.length > 0) {
    lines.push('Consider adding (themes MMD looked for and did not find):');
    for (const { theme, suggestion } of missing) {
      lines.push(`- ${theme} — e.g. ${suggestion}`);
    }
  } else {
    lines.push('No gaps detected among the themes MMD checks — nice and thorough.');
  }
  return lines;
}
 ...
 * @param {string|null} [args.constitutionText]  the project's constitution text
 *        (when present, a non-destructive "Constitution suggestions" section is
 *        rendered; null/absent → section omitted, back-compat with pre-v0.6.b)
 * @param {Date}   [args.clock]         injected timestamp (defaults to new Date())
 * @returns {string}
 */
export function buildReport({ targetDir, scanData, ingestData, inferredMd, caseLabel, version, constitutionText = null, clock }) {
  ...
  lines.push('');
  lines.push(`> Generated by \`mmdream discover\` at ${now.toISOString()} on ${targetDir}`);
  lines.push('> Status: PENDING VALIDATION — run `mmdream discover --approve` after review');
  ...
  lines.push('');
  const suggestionLines = constitutionSuggestionsSection(constitutionText);
  if (suggestionLines.length > 0) {
    for (const l of suggestionLines) lines.push(l);
    lines.push('');
  }
  ...
  lines.push('---');
  lines.push('');
  lines.push('*This report was generated automatically. Edit it freely to correct errors; re-run `mmdream discover --refresh` to regenerate from scratch.*');
  lines.push('');
  return lines.join('\n');
}
 ...
 * VALIDATED at <iso>. Used by `mmdream discover --approve`. The match is robust
