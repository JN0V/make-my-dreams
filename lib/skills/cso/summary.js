// lib/skills/cso/summary.js — summary + dry-run formatters for `mmdream cso`
...
export function formatCsoSummary(info) {
  ...
  return [
    ...
    '',
    'Note: mmdream cso is read-only / advisory — no commits / no pushes were made.',
    '',
  ].join('\n');
}
...
export function formatCsoDryRun(info) {
  ...
  return [
    '',
    '═══ mmdream cso --dry-run — preview (no subprocess spawned) ═══',
    `  Branch     : ${info.branch}`,
