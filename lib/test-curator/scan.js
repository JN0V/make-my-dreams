...
// AC-1 (v0.7.7): the redundancy face needs each test's BODY (for structural
// similarity) and each file's TARGETS (the lib/bin modules it imports, for
// clustering). Both come from the pure extractor — scan.js stays the single
// place that turns raw {path, content} into structured entries.
import { extractTestBody, extractFileTargets } from './extract-bodies.js';
...
function scanOne(filePath, content) {
  ...
  const tests = [];
  // The file's targets (lib/bin modules it imports) — computed once, attached to
  // every test in the file so the redundancy clustering can group by target.
  const targets = extractFileTargets(content);
  // Running char offset of the start of the current line, so we can map a
  // line-local match to an absolute index into `content` for the body scan.
  let lineOffset = 0;
  for (let li = 0; li < lines.length; li += 1) {
    ...
    if (!isCommentLine(line)) {
      ...
      while ((m = TEST_CALL_RE.exec(line)) !== null) {
        ...
        // Body: brace-scan from this test call's absolute offset (skips the title
        // string + comments so a brace inside the title never opens the body).
        const body = extractTestBody(content, lineOffset + m.index);
        tests.push({
          title,
          tag: tagOf(title),
          file: filePath,
          line: li + 1, // 1-based
          body,
          targets, // the file's lib/bin imports (shared by all tests in the file)
        });
  ...
  return {
    tests,
    metric: { path: filePath, lineCount: lines.length, testCount: tests.length, targets },
  };
}
 ...
 * @returns {{
 *   tests: Array<{ title: string, tag: string, file: string, line: number, body: string, targets: string[] }>,
 *   files: Array<{ path: string, lineCount: number, testCount: number, targets: string[] }>,
  ...
  if (Array.isArray(files)) {
    for (const entry of files) {
      ...
      if (content === null) {
        ...
        // (honest — it exists but contributed no tests), no tests, no targets.
        fileMetrics.push({ path: filePath, lineCount: 0, testCount: 0, targets: [] });
        continue;
      }
