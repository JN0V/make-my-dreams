...
// (SPEC_V077 AC-2). The second face of `mmdream test-health`: find tests that likely
...
//
// LANGUAGE-NEUTRAL (§VIII): this module is part of the generic core. It operates
// ONLY on the normalized entry fields (`body`, `targets`, `file`, `line`,
// `title`) the adapters produce — it references NO language syntax and imports NO
// adapter. A `body` the adapter could not extract is null → tokenizes to nothing
// → skipped (the report renders the honest capability note); `targets` are
// opaque module paths the adapter resolved (JS `lib/x.js`, Python `pkg/mod.py`,
// …), grouped without interpretation here.
...
/**
 * Group test files by the project module(s) they target and
