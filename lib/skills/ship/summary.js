...
/**
 * Format the final summary printed after `mmdream ship` completes.
 *
...
export function formatDryRun(info) {
  ...
  const envLines = Object.keys(safeEnv)
    .sort()
    .map((k) => `    ${k}=${safeEnv[k]}`);

  return [
    '',
    '═══ mmdream ship --dry-run — preview (no subprocess spawned) ═══',
    `  Branch     : ${info.branch}`,
