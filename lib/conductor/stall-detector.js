...
// a well-formed result for `mmdream unblock` to act on.
...
export const DEFAULT_THRESHOLDS = Object.freeze({
  ...
  // SPEC_V02N AC-3: minutes a slice's worktree may stay dirty (uncommitted)
  // past the last commit before we raise wip-uncommitted-since-N-min. Derived
  // from lastCommitAge (git records no "dirty-since" timestamp). 15 min is the
  // L-019 WIP-loss-on-kill window. Overridable via MMD_STALL_WIP_UNCOMMITTED_MIN.
  wipUncommittedMin: 15,
});
...
export function resolveThresholds(env = process.env, override = {}) {
  ...
  const fromEnv = {
    ...
    wipUncommittedMin: num(env.MMD_STALL_WIP_UNCOMMITTED_MIN, DEFAULT_THRESHOLDS.wipUncommittedMin),
  };
...
/**
 * Default worktree-status accessor (SPEC_V02N AC-2): true when the repo's
 * working tree carries uncommitted changes (`git status --porcelain` is
 * non-empty), false when clean, null when git is unavailable / errored. Never
 * throws — mirrors defaultGitLastCommitEpoch's contract so the detector always
 * returns a well-formed result. `branch` is accepted for signature symmetry
 * with the other git accessors; `git status` is inherently working-tree-scoped.
 *
 * @param {string} repoRoot
 * @param {string} branch
 * @returns {boolean|null}
 */
function defaultGitWorktreeDirty(repoRoot, branch) { // eslint-disable-line no-unused-vars
  try {
 ...
 */
export function readRecentRunLogs(repoRoot, maxFiles = 5) {
 ...
 *   gitWorktreeDirtyFn?: (repoRoot: string, branch: string) => boolean|null,
...
export function detectStall(opts) {
  ...
  const {
    ...
    gitWorktreeDirtyFn = defaultGitWorktreeDirty,
    readRunLogsFn = (root) => readRecentRunLogs(root),
  } = opts;
  ...
  const evidence = {
    ...
    // SPEC_V02N AC-2: worktree-dirty + derived WIP age. null until resolved.
    worktreeDirty: null,
    wipUncommittedMin: null,
    thresholds,
    errors,
  };
  ...
  // ── worktree-dirty + WIP age (SPEC_V02N AC-2) ──────────────────────────────
  // Fire wip-uncommitted-since-N-min ONLY when the tree is dirty AND the last
  // commit is older than the threshold. A clean tree (worktreeDirty === false)
  // or a never-committed branch (lastCommitAgeMin === null) is NOT a WIP-loss
  // risk and must not fire. WIP age is derived from lastCommitAge — git records
  // no "dirty-since" timestamp (ADR-018; non-feature in SPEC §4).
  let worktreeDirty = null;
  try {
    worktreeDirty = gitWorktreeDirtyFn(repoRoot, sliceBranch);
  } catch (err) {
    // Never crash: an injected/real accessor that throws → null + evidence
    // trail, exactly like the gitLastCommitEpochFn catch above.
    errors.push(`git worktree-dirty lookup failed: ${err.message}`);
    worktreeDirty = null;
  }
  evidence.worktreeDirty = worktreeDirty === true ? true : worktreeDirty === false ? false : null;
  if (worktreeDirty === true && Number.isFinite(evidence.lastCommitAgeMin)) {
    // Derived: a dirty tree whose last commit is N min old ⇒ WIP uncommitted ≥ N min.
    evidence.wipUncommittedMin = evidence.lastCommitAgeMin;
    if (evidence.lastCommitAgeMin > thresholds.wipUncommittedMin) {
      signals.push('wip-uncommitted-since-N-min');
    }
  }
