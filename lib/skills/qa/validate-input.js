// lib/skills/qa/validate-input.js — pre-flight checks for `mmdream qa`
...
export async function validateQaTarget(cwd, opts = {}) {
  if (typeof cwd !== 'string' || cwd.length === 0) {
    return { ok: false, exitCode: 3, message: 'mmdream qa: internal error — cwd is required' };
  }
  ...
  if (!inside.ok) {
    return {
      ...
      exitCode: 3,
      message:
        `mmdream qa requires the current directory to be a git repository, ` +
  ...
  if (!branch) {
    ...
    if (!cur.ok || cur.code !== 0) {
      return {
        ok: false,
        exitCode: 3,
        message: `mmdream qa: could not read current branch: ${
  ...
  if (!branch || branch.length === 0) {
    return {
      ...
      exitCode: 3,
      message:
        'mmdream qa: current branch is empty — detached HEAD? ' +
  ...
  if (/\s/.test(branch) || branch.startsWith('-')) {
    ...
    return {
      ok: false,
      exitCode: 2,
      message: `mmdream qa: refusing branch with suspicious characters: '${branch}'`,
    };
  }
  ...
  if (!rev.ok || rev.code !== 0) {
    return {
      ok: false,
      exitCode: 3,
      message: `mmdream qa: could not read HEAD SHA: ${
