...
export function renderCoherenceReport(args) {
  const {
    ...
    generatedBy = 'mmd document-review',
    version = null,
  } = args || {};
  ...
  parts.push('');
  parts.push(
    '> Generated on demand by `mmd document-review` — the Documentalist\'s **detection** face: ' +
    ...
    'change (new subcommand, ADR, tag, or `MAKE_MY_DREAMS.md` edit) with `mmd document-review`. ' +
    'The designed-vs-built table is a **heuristic** (name-matching), not an authoritative audit; ' +
    'run `mmd document-review --with-claude` to layer LLM judgment on top.',
  );
  if (version) {
    parts.push('');
