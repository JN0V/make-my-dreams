...
// Catch-route body cap (F1). A synthesized/edited scope may run up to
// MAX_SCOPE_LEN (4000) chars; POST /api/catch/edit ships it as
// {"sessionId":"<uuid>","scope":"..."} and JSON-escaping can blow each char up
// to 6 bytes (\uXXXX), so a 4000-char scope can reach ~24 KB on the wire. A
// 32 KB cap holds that worst case with room to spare while still bounding DoS.
// The legacy POST /api/dream stays on the tight 4 KB cap (its dream is ≤500).
