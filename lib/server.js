...
export async function createServer({ port, explicitPort, cwd }) {
  ...
  // POST /api/catch/answer {sessionId, answer:profile} → {next:"scope", scope, profile, fallback}
  async function handleCatchAnswer(req, res, requestId) {
    ...
    if (entry.session.state !== 'profile') {
      return jsonResponse(res, 409, { error: 'bad_session_state', state: entry.session.state });
    }
    // F1 — single in-flight synthesize: reject a second concurrent synthesize so
    // a start+answer loop cannot spawn unbounded claude -p subprocesses.
    if (synthesizing) {
      return jsonResponse(res, 409, { error: 'synthesize_in_progress' });
    }
    synthesizing = true;
    // The autonomous synthesize is a headless claude -p call that can run for
    // tens of seconds — longer than the 30 s per-request timeout. Disable it
    // for this long-poll (mirrors the SSE handler's res.setTimeout(0)).
    res.setTimeout(0);
    let result;
    try {
      result = await entry.session.setProfile(payload.answer);
    } catch (err) {
    ...
    logEvent('info', 'catch_synthesized', {
      request_id: requestId, profile: result.profile, fallback: result.fallback,
    });
