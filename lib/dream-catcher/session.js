...
/**
 * THE single shared ceiling on a synthesized OR hand-edited scope (F1).
 *
 * Real `bmad-product-brief` scopes are multi-paragraph and routinely exceed the
 * 500-char dream cap, so the scope must have its OWN, larger ceiling — used
 * consistently in three places so a scope can NEVER be longer than what every
 * layer accepts:
 *   (a) here — bounding the synthesized scope on the way in (boundScope below,
 *       called from runSynthesize) and the hand-edited scope (editScope);
 *   (b) lib/server.js — the POST /api/catch/edit validation imports this value;
 *   (c) bin/serve-ui/index.html — the edit <textarea maxlength> matches it.
 *
 * Body-cap math (F1): POST /api/catch/edit sends
 *   {"sessionId":"<36-char uuid>","scope":"<up to MAX_SCOPE_LEN chars>"}
 * Worst case every scope char escapes to 6 bytes (\uXXXX), so the JSON body can
 * reach ~6 × MAX_SCOPE_LEN + overhead. With MAX_SCOPE_LEN = 4000 that is up to
 * ~24 KB, which does NOT fit the legacy 4 KB body cap — so lib/server.js raises
 * the catch-route body cap to 32 KB (CATCH_MAX_BODY_BYTES) to hold it with room
 * to spare. The legacy POST /api/dream cap stays 4 KB.
 */
export const MAX_SCOPE_LEN = 4000;

/**
 * Truncate a string to the shared scope ceiling (F1). Pure helper so both the
 * synthesize path and any future caller bound a scope identically. Never throws
 * — over-long synthesized output is trimmed (honest: it is still the real
 * synthesized text, just capped), never rejected mid-flow.
 *
 * @param {string} text
 * @returns {string}
 */
function boundScope(text) {
  if (text.length <= MAX_SCOPE_LEN) return text;
  let cut = text.slice(0, MAX_SCOPE_LEN);
  // Don't leave a dangling high surrogate if the cut landed inside a surrogate
  // pair (e.g. an emoji straddling the boundary) — it would render as U+FFFD.
...
export function createSession({ elicit } = {}) {
  ...
  async function runSynthesize({ mode }) {
    ...
    if (result && typeof result.scope === 'string' && result.scope.trim().length > 0) {
      // Bound the synthesized scope to the shared ceiling on the way IN (F1) so a
      // scope can never exceed what /api/catch/edit and the edit textarea accept.
      scope = boundScope(result.scope);
