// lib/skills/qa/summary.js — pure formatting of the post-`mmdream qa` summary
...
/**
 * Format the summary printed after `mmdream qa` completes.
 *
 * @param {{
 *   branch: string,
 *   baseBranch: string,
...
export function formatQaSummary(info) {
  ...
  return [
    ...
    '',
    'Note: mmdream qa is read-only / advisory — no commits / no pushes were made.',
    '',
  ].join('\n');
}
...
export function formatQaDryRun(info) {
  ...
  return [
    '',
    '═══ mmdream qa --dry-run — preview (no subprocess spawned) ═══',
    `  Branch     : ${info.branch}`,
