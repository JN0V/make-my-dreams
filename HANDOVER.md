...
> Updated: 2026-06-02, after a long single session that landed **v0.6.0 → v0.8.0** (14 versions): third-party readiness (works on other repos), the full **Documentalist** (detect gaps / drift / compact SPECs / coherence graph + hub-cap), the **`/mmd`** Claude slash command, the **Test Curator** (`mmdream test-health`: stratification + redundancy + cluster precision), and the **polyglot §VIII correction** (the Test Curator was secretly JavaScript-only → now adapter-based, proven on Python, honest-refuses Rust; constitution `universal §VIII` added as the guardrail).
...
**This session's arc (v0.6.0 → v0.8.0):** third-party readiness → the full Documentalist (`mmdream document-review` detect + drift/conformance + `mmdream document-compact` SPEC archival + `--since` coherence graph + hub-cap) → `/mmd` operator slash command (live in `.claude/commands/`) → the Test Curator (`mmdream test-health`: corpus health + redundancy + cluster precision) → **the §VIII polyglot correction** (Sébastien caught that the Test Curator was JS-only; fixed via adapters + the new constitution §VIII guardrail; proven on Python, refuses Rust honestly). Each version has a detailed "JUST LANDED" block below and a changelog entry in README.
...
- ✅ **`/mmd` slash command (v0.7.5)** — the operator playbook as a Claude Code command (`assets/claude-commands/mmdream.md`, materialized by install-mmd.sh into `.claude/commands/mmdream.md`). Drive MMD by intent from a session. Live in this repo (materialized).
