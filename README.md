...
> **Run `mmdream serve` from a directory where MMD is installed.** The dream flow scopes via the BMAD `bmad-product-brief` skill (and builds with auto-dev) — both installed per-project by `install-mmd.sh`. If you launch `serve` (or greenfield `mmdream "<dream>"`) from a directory without BMAD, MMD **fails fast with an honest setup message** (exit 8) instead of a cryptic mid-dream error: either run `bash <your-MMD-install>/install-mmd.sh .` there, or launch from your MMD install directory. (`MMD_SKIP_SETUP=1` bypasses the check.)
