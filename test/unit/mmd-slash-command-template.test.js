// @unit anchor-presence tests for the /mmd Claude Code slash-command template
// (v0.7.5). The template at assets/claude-commands/mmd.md is the TRACKED SOURCE
// that install-mmd.sh materializes into .claude/commands/mmd.md. These cheap
...
const TEMPLATE_REL = 'assets/claude-commands/mmd.md';
const TEMPLATE_ABS = path.join(REPO_ROOT, TEMPLATE_REL);

test('@unit /mmd template: tracked source exists outside .claude/', () => {
...
test('@unit /mmd template: has slash-command frontmatter (name + description)', () => {
...
test('@unit /mmd template: uses the Claude Code argument placeholder', () => {
  const md = readFileSync(TEMPLATE_ABS, 'utf8');
  assert.match(md, /\$ARGUMENTS/, 'must reference $ARGUMENTS so /mmd <intent> routes the intent');
});

test('@unit /mmd template: encodes the detached-launch operational rules', () => {
  const md = readFileSync(TEMPLATE_ABS, 'utf8');
...
test('@unit /mmd template: requires commit-incrementally-per-AC in the dream (L-019)', () => {
  const md = readFileSync(TEMPLATE_ABS, 'utf8');
  assert.match(md, /commit incrementally per AC/i);
  assert.match(md, /L-019/);
});

test('@unit /mmd template: carries the spec-frozen directive for frozen SPECs', () => {
  const md = readFileSync(TEMPLATE_ABS, 'utf8');
...
test('@unit /mmd template: monitors via status.json + git commits, not the buffered log (L-002)', () => {
  const md = readFileSync(TEMPLATE_ABS, 'utf8');
...
test('@unit /mmd template: reports honestly + verifies before declaring done', () => {
  const md = readFileSync(TEMPLATE_ABS, 'utf8');
...
test('@unit /mmd template: on green offers ff-only merge + annotated tag', () => {
  const md = readFileSync(TEMPLATE_ABS, 'utf8');
  assert.match(md, /--ff-only/, 'merge must be fast-forward only');
  assert.match(md, /tag -a/, 'tag must be annotated');
});

test('@unit /mmd template: documents all three routes (code change / greenfield / bare subcommand)', () => {
  const md = readFileSync(TEMPLATE_ABS, 'utf8');
