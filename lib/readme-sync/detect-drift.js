...
// whole `mmd <name>` invocation first, then as a bare `<name>` token, so a doc
// that writes ``mmd qa`` or just mentions the `qa` skill both count as covered.
...
export function detectDrift(opts) {
  ...
  const missingSubcommands = subcommands.filter((name) => {
    ...
    // Covered if the README mentions `mmd <name>` OR the bare token.
    return !text.includes(`mmd ${name}`) && !text.includes(name);
  });
