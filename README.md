...
It is **deterministic** (no LLM — the corpus signal is exactly computable) and a clearly-labelled **heuristic**, not an audit. Pure scanner + render in `lib/test-curator/{scan,report}.js`, body/target extraction in `extract-bodies.js`, similarity + clustering in `redundancy.js`; the thin read-only bin in `bin/test-curator/test-health.js`. See [ADR-040](./docs/adr/040-test-curator-test-health.md) and [ADR-041](./docs/adr/041-test-curator-redundancy-detection.md).
...
- **Reflexive slices (release tags)**: 39
- **Tests**: 1747 passing
