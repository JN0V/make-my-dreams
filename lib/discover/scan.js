...
import { detectTestRunnerFromScripts } from './test-runner-detector.js';
 ...
 * docs/adr / .mmdream / CLAUDE.md / MAKE_MY_DREAMS.md). We also surface a
...
export async function detectFrameworks(targetDir) {
  ...
  if (await pathExists(pkgPath)) {
    ...
    try {
      ...
      // L-017 (AC-1): when no devDependency declares the runner, read
      // package.json.scripts — catches `"test": "node --test ..."` (MMD's own
      // shape) and the full runner set a project may invoke without a matching
      // devDependency. Dep-declared runners keep precedence (unchanged behavior).
      if (!test_runner) test_runner = detectTestRunnerFromScripts(pkg.scripts);
...
/* ───────────────────────── test-file counting (L-017, AC-2) ────────────────── */

/** Directories conventionally holding tests. Each is walked recursively. */
const TEST_DIRS = Object.freeze(['test', 'tests', '__tests__', 'spec', 'specs']);

/**
 * Match a test FILE by name (AC-2 glob):
 *   **\/*.{test,spec}.{js,ts,jsx,tsx,py,rs,go}  +  **\/*_test.{py,go}
 */
const TEST_FILE_RE =
  /(?:\.(?:test|spec)\.(?:js|ts|jsx|tsx|py|rs|go)|_test\.(?:py|go))$/;

/** Dirs never descended into while counting (gitignore-standard ignores). */
const TEST_WALK_SKIP = new Set([
  '.git', 'node_modules', 'dist', 'build', '.mmd',
  '_bmad', '_bmad-output', '.specify',
]);

/**
 * Recursively count test files under the conventional test directories
 * (AC-2, L-017). Bounded to `maxDepth` levels below each test root. NEVER
 * follows symlinks (security: a symlinked dir could escape the target or loop).
 * Skips the standard ignored dirs so vendored/build output is never counted
 * (gitignore-aware in the practical sense AC-2 requires).
 *
 * @param {string} targetDir absolute path
 * @param {{ maxDepth?: number }} [opts]
 * @returns {Promise<{ count: number, dirs: string[] }>}
 */
export async function countTestFiles(targetDir, { maxDepth = 4 } = {}) {
  let count = 0;
  const dirsWalked = [];
  ...
  async function walk(dir, depth) {
    ...
    for (const e of entries) {
      ...
      if (TEST_WALK_SKIP.has(e.name)) continue;
      const full = path.join(dir, e.name);
      if (e.isDirectory()) {
  ...
  for (const td of TEST_DIRS) {
    const root = path.join(targetDir, td);
    if (await isDir(root)) {
      dirsWalked.push(td);
      await walk(root, 1);
    }
  }
  return { count, dirs: dirsWalked };
}
...
export async function runScan(targetDir, opts = {}) {
  const [methodologies, languages, frameworks, lint, git, already_onboarded, tests] =
    await Promise.all([
