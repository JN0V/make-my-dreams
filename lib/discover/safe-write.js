...
export async function assertSafeWritePath(targetDir, candidatePath, opts = {}) {
  ...
  // SPEC_V06B AC-3: discover ensures its scratch outputs are gitignored, so the
  // root `.gitignore` is now an allowed sink (append-only, marked block — the
  // caller never clobbers existing entries and never touches constitution.md).
  const gitignorePath = absTarget + sep + '.gitignore';
  ...
  const isGitignore = absCandidate === gitignorePath;

  if (!(underMmd || isReport || underDocs || isGitignore)) {
    throw new Error(
      `non-intrusion violation: '${absCandidate}' is not under .mmd/, ` +
      `docs/, the root mmd-discovery-report.md, or the root .gitignore`,
    );
  }
