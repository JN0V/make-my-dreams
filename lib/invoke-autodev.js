...
import { modelForRole } from './conductor/model-policy.js';
 ...
 * conductor-default contract is unit-testable (SPEC_V015A AC-1, ADR-054).
 *
 * v0.15.a — the transparent Conductor: the monitored (stream-json) spawn is now
 * the DEFAULT, so `monitor` DEFAULTS TO `true` and the default CLI args INCLUDE
 * `--output-format stream-json --verbose`. The pre-v0.15 historical shape is the
 * OPT-OUT, reached by passing `monitor: false` (which bin/mmd.js resolves from
 * `MMD_NO_AUTO_HANDOFF=1` via resolveConductorMode — the bootstrap/cost escape
 * hatch). The earlier byte-for-byte-default-spawn contract was self-imposed
 * caution; stream-json changes only the OUTPUT FORMAT, not the agent's work.
 *
 *   CLI, default (monitor)  → ['-p', '/bmad-adv-auto-dev <prompt>', '--output-format', 'stream-json', '--verbose']
 *   CLI, opt-out (monitor:false) → ['-p', '/bmad-adv-auto-dev <prompt>']  (historical, byte-for-byte)
 ...
 * @returns {string[]}
 */
export function buildAutodevArgs({ isClaudeCli, prompt, dream, monitor = true, model = null }) {
...
export function makeMonitorConsumer(logStream, quiet, onContext) {
  ...
  const handleLine = (line) => {
    ...
    // v0.16.1 (monitor-accuracy fix): count a usage reading as a context-SIZE
    // snapshot ONLY from a per-message `assistant` event (input + cache for that
    // single API call — bounded by the window). The `result` event's top-level
    // usage is a CUMULATIVE run-total (cache_creation summed across every
    // iteration), so counting it made the monitor report an absurd context %
    // (e.g. 1546% / 15M tokens on a 1M window) and falsely cross the handoff
    // threshold. The max of the per-message readings IS the real peak context.
    if (ev.usage && ev.type === 'assistant') {
...
export async function invokeAutodev({
  ...
  // v0.16.a (AC-3): model-per-role — the auto-dev PARENT is the ORCHESTRATOR
  // role. It coordinates + delegates (it hands off at 70% so it never needs a
  // big window), so it defaults to the LIGHT model (sonnet) via the policy. The
  // global MMD_AUTODEV_MODEL still overrides everything (kept as the explicit
  // operator escape hatch); the policy itself honors MMD_MODEL_ORCHESTRATOR.
  const orchestratorModel = process.env.MMD_AUTODEV_MODEL || modelForRole('orchestrator', process.env);
  const args = buildAutodevArgs({
    isClaudeCli, prompt, dream, monitor, model: orchestratorModel,
  });
