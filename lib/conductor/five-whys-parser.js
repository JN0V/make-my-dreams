...
/**
 * Find the last balanced {...} substring by scanning from the last '{' that
 * yields a balanced object. Brace-counting (string-aware) avoids greedy/lazy
 * regex pitfalls on nested objects.
 *
 * @param {string} text
 * @returns {string|null}
 */
function lastBalancedObject(text) {
  ...
  for (let start = text.length - 1; start >= 0; start -= 1) {
    ...
    const candidate = balancedFrom(text, start);
    if (candidate) {
      result = candidate;
      break; // first balanced object found scanning backwards = the last one
    }
  }
  return result;
}
