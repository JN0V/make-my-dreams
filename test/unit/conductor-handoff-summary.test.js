...
test('@unit AC-1: summarizeHandoffState is exported as a function', () => {
  assert.strictEqual(typeof summarizeHandoffState, 'function');
});

test('@unit AC-1: HANDOFF_SUMMARY_TOTAL_PHASES is exported as 4', () => {
  assert.strictEqual(HANDOFF_SUMMARY_TOTAL_PHASES, 4);
  assert.strictEqual(typeof HANDOFF_SUMMARY_TOTAL_PHASES, 'number');
});

// ── AC-2 — null / undefined checkpoint ───────────────────────────────────────

test('@unit AC-2: null checkpoint returns "No checkpoint — run not started"', () => {
  assert.strictEqual(summarizeHandoffState(null, 0), 'No checkpoint — run not started');
});

test('@unit AC-2: undefined checkpoint returns "No checkpoint — run not started"', () => {
  assert.strictEqual(summarizeHandoffState(undefined, 0), 'No checkpoint — run not started');
});

test('@unit AC-2: null checkpoint with non-zero handoffsSoFar still returns no-checkpoint string', () => {
  assert.strictEqual(summarizeHandoffState(null, 5), 'No checkpoint — run not started');
});

// ── AC-3 — empty / phase-0 / invalid phase checkpoint ────────────────────────

test('@unit AC-3: empty object checkpoint returns "No phase completed yet"', () => {
  assert.strictEqual(summarizeHandoffState({}, 0), 'No phase completed yet');
});

test('@unit AC-3: lastCompletedPhase: 0 treated the same as empty (phase 0 means no boundary reached)', () => {
  assert.strictEqual(summarizeHandoffState({ lastCompletedPhase: 0 }, 0), 'No phase completed yet');
});

test('@unit AC-3: lastCompletedPhase: NaN returns "No phase completed yet"', () => {
  assert.strictEqual(summarizeHandoffState({ lastCompletedPhase: NaN }, 0), 'No phase completed yet');
});

test('@unit AC-3: lastCompletedPhase: "garbage" (string) returns "No phase completed yet"', () => {
  assert.strictEqual(summarizeHandoffState({ lastCompletedPhase: 'garbage' }, 0), 'No phase completed yet');
});

test('@unit AC-3: lastCompletedPhase: -1 (negative) returns "No phase completed yet"', () => {
  assert.strictEqual(summarizeHandoffState({ lastCompletedPhase: -1 }, 0), 'No phase completed yet');
});

// ── AC-4 — phases 1, 2, 3 in-progress ────────────────────────────────────────

test('@unit AC-4: phase 1 returns "Phase 1/4 complete"', () => {
  assert.strictEqual(summarizeHandoffState({ lastCompletedPhase: 1 }, 0), 'Phase 1/4 complete');
});

test('@unit AC-4: phase 2 returns "Phase 2/4 complete"', () => {
  assert.strictEqual(summarizeHandoffState({ lastCompletedPhase: 2 }, 0), 'Phase 2/4 complete');
});

test('@unit AC-4: phase 3 returns "Phase 3/4 complete"', () => {
  assert.strictEqual(summarizeHandoffState({ lastCompletedPhase: 3 }, 0), 'Phase 3/4 complete');
});
...
test('@unit AC-6: phase 4 with 0 handoffs returns exact "Run complete (4/4 phases)"', () => {
  assert.strictEqual(summarizeHandoffState({ lastCompletedPhase: 4 }, 0), 'Run complete (4/4 phases)');
});
...
test('@unit AC-6: phase 99 (overflow) capped at "Run complete (4/4 phases)"', () => {
  assert.strictEqual(summarizeHandoffState({ lastCompletedPhase: 99 }, 0), 'Run complete (4/4 phases)');
});
...
test('@unit AC-8: (Symbol, Symbol) does not throw, returns "No phase completed yet", no "NaN"', () => {
  ...
  assert.strictEqual(result, 'No phase completed yet');
...
// ── Boundary cases not in spec ACs but pinned to prevent regression ──────────

test('@unit boundary: lastCompletedPhase=0.5 (fractional, < 1) returns "No phase completed yet"', () => {
  assert.strictEqual(summarizeHandoffState({ lastCompletedPhase: 0.5 }, 0), 'No phase completed yet');
});

test('@unit boundary: handoffsSoFar=0.9 (finite but < 1) treated as 0, no handoff mention', () => {
  const result = summarizeHandoffState({ lastCompletedPhase: 2 }, 0.9);
  assert.ok(!result.includes('handoff'), `Expected no "handoff" in: ${result}`);
});

test('@unit boundary: BigInt lastCompletedPhase does not throw and returns "No phase completed yet"', () => {
  let result;
  assert.doesNotThrow(() => { result = summarizeHandoffState({ lastCompletedPhase: BigInt(2) }, 0); });
  assert.strictEqual(result, 'No phase completed yet');
  assert.ok(!result.includes('NaN'), `Output must not contain "NaN": ${result}`);
});
