...
export async function createServer({ port, explicitPort, cwd }) {
   ...
   */
  function launchJobAndRespond(res, requestId, { dream, slug }) {
  ...
  async function handleCatchConfirm(req, res, requestId) {
    ...
    // field is authoritative only until the subprocess rewrites it. Threading
    // the profile INTO the subprocess (MMD_PROFILE) is deferred to v0.3.a-2
    // (SPEC_V03A1 §4 out-of-scope). See ADR-021.
    try {
    ...
    const launched = launchJobAndRespond(res, requestId, { dream: scope, slug });
    if (launched) {
