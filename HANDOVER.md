...
**Documentalist roadmap (the main thrust — v0.7.a landed the roadmap-level DETECT face). Core mission (Sébastien, 2026-06-01): keep docs TRUE first (conformance / anti-drift), tidy second (compaction). Truth over tidiness.**
- **v0.7.b — DRIFT / conformance detection (THE PRIORITY).** Per-claim "is the doc still true?": `mmd document-review` gains a "Drift / conformance" section flagging dangling artifact references (a doc cites a `lib/…`/`bin/…` file, `mmd <subcmd>`, `ADR-NNN`, or `lib/<module>` that no longer exists) + bounded stale facts (count/version claims vs the live inventory), + opt-in `--with-claude` semantic drift (SPEC/ADR description vs the code's actual behavior). Mostly deterministic (reuse the v0.7.a inventory). **Flag-only — detect-before-correct** (rewriting prose to fix drift is a trust-gated later step). Precision-first (no crying wolf). docs/specs/SPEC_V07B.md drafted.
- **v0.7.c — active compaction (the "stay tidy" half).** Archive the 33 root `SPEC_V*.md` into `docs/specs/` + index (git mv, reference rewrites, idempotent, reversible) — `mmd document-compact`; then the harder semantic doc-sharding (`MAKE_MY_DREAMS.md` 1722 lines, etc.). "Le balayage fait par le Documentaliste lui-même." (A compaction SPEC was drafted then reassigned here when v0.7.b was re-scoped to drift.)
- **v0.7.d — the coherence graph (Sébastien's idea, 2026-06-01).** A *derived* (never hand-maintained) traceability graph coupling doc↔code↔ADR blocks with **staleness propagation on diff**: change one node → linked nodes flagged "may be stale, review." Reuse `computeBlastRadius` (ADR-027, code↔code), the grounding file-ref extractor (doc→code), `[[name]]`/`ADR-NNN`/md links (doc↔doc) — ~80% of edges derive for free. Opt-in n-ary *semantic* link = a micro-anchor `@mmd:link <concept>`. Value = a diff-walk check (`--since <ref>`), **advisory + ranked, never a hard gate**. Start file/symbol/anchor granularity. Bonus: Mermaid/.dot mind-map render. Own design conversation first. See memory `coherence-graph-traceability`.
