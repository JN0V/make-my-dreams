...
// `mmdream document-release` (SPEC_V02G AC-4 + AC-5).
//
...
export async function validateDocumentReleaseTarget(cwd, opts = {}) {
  if (typeof cwd !== 'string' || cwd.length === 0) {
    return { ok: false, exitCode: 3, message: 'mmdream document-release: internal error — cwd is required' };
  }
  ...
  if (!inside.ok) {
    return {
      ...
      exitCode: 3,
      message:
        `mmdream document-release requires the current directory to be a git ` +
  ...
  if (fromRef === null) {
    ...
    if (!desc.ok) {
      return {
        ok: false,
        exitCode: 4,
        message: `mmdream document-release: could not auto-detect last tag: ${desc.error.message}`,
      };
    }
    if (desc.code !== 0) {
      return {
        ...
        exitCode: 4,
        message:
          'mmdream document-release: no tags found in this repo — ' +
          'pass <from> explicitly (e.g. `mmdream document-release HEAD~10 HEAD`).',
      };
    }
    ...
    if (!fromRef) {
      return {
        ok: false,
        exitCode: 4,
        message: 'mmdream document-release: auto-detected last tag was empty — pass <from> explicitly.',
      };
    }
  }
  ...
  if (isSuspiciousRef(fromRef)) {
    return {
      ok: false,
      exitCode: 4,
      message: `mmdream document-release: refusing suspicious <from> ref: '${fromRef}'`,
    };
  }
  if (isSuspiciousRef(toRef)) {
    return {
      ok: false,
      exitCode: 4,
      message: `mmdream document-release: refusing suspicious <to> ref: '${toRef}'`,
    };
  }
  ...
  if (!fromRev.ok || fromRev.code !== 0) {
    return {
      ...
      exitCode: 4,
      message:
        `mmdream document-release: <from> ref '${fromRef}' is not a valid commit: ` +
  ...
  if (!toRev.ok || toRev.code !== 0) {
    return {
      ...
      exitCode: 4,
      message:
        `mmdream document-release: <to> ref '${toRef}' is not a valid commit: ` +
